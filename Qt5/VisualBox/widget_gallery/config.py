...
from widget_gallery.util import OPERATORS, compare_v, get_logger, multi_replace

_logger = get_logger(__name__)

_qt_version = "10.0.0"
_qt_api = QT_API = "PyQt5"

_RESOURCE_HOME_DIR = Path.home() / ".qdarktheme"
_RESOURCES_BASE_DIR = _RESOURCE_HOME_DIR / f"{__version__}"
...
def _build_svg_files(theme: str, theme_resources_dir: Path) -> None:
    ...
    if theme == "dark":
        from themes.dark.svg import SVG_RESOURCES
    else:
        from themes.light.svg import SVG_RESOURCES
...
def load_stylesheet(theme: str = "dark", border: str = "rounded") -> str:
    ...
    Examples:
        ...
        Sharp Frame::
            ...
            app = QApplication([])
            app.setStyleSheet(load_stylesheet(border="sharp"))
    """
    ...
    if theme == "dark":
        from themes.dark.stylesheet import STYLE_SHEET
    else:
        from themes.light.stylesheet import STYLE_SHEET
...
def load_palette(theme: str = "dark"):
    ...
    Examples:
        ...
        2. Light Theme::
            ...
            app = QApplication([])
            app.setPalette(load_palette("light"))
    """
    if theme not in get_themes():
        raise TypeError("The argument [theme] can only be specified as 'dark' or 'light'.") from None

    if theme == "dark":
        from themes.dark.palette import PALETTE
    else:
        from themes.light.palette import PALETTE
    return PALETTE
